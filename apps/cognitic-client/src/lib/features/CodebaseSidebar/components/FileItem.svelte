<script lang="ts">
  ...
  $: type = file.filePath.slice(file.filePath.lastIndexOf('.') + 1) || 'file';
</script>
...
>
  <Checkbox
    ...
    checkboxClass="w-3 h-3 ml-2 flex-shrink-0"
    checked={selected}
    on:click
  >
    <div slot="before" class="flex flex-1 items-center overflow-hidden">
      <svg
        viewBox="0 0 13 16"
        fill="none"
      ...
      <span class="body-regular text-content-primary ml-2">{file.fileName}</span
      >
    </div>
  </Checkbox>
</div>
